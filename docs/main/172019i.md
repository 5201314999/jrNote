...
```
都是在response header 中返回。
字段记忆不清晰.
(1) 强缓存cache-control： max-age(缓存最长时间) ，expires(http1.0 版本)  public 代理服务器/ private 客户端 immutable:它这么设置的意思是为了让用户在刷新页面的时候不要去请求服务器 no-cache 跳过设置强缓存，不影响协商缓存   no-store 禁用所有缓存
